...
# # def unflatten_weights(flattened_weights, original_shapes):
# #     """Convert flat list into the model's original nested tensors."""
# #     print("Flattened weights length:", len(flattened_weights))
...
# #         num_elements = np.prod(shape)
# #         current_elements = flattened_weights[current_index : current_index + num_elements]
# #         current_elements = np.array(current_elements)
# #         print(f"Current elements shape: {current_elements.shape}")
# #         reshaped_elements = np.reshape(current_elements, shape)
# #         print(f"Reshaped elements shape: {reshaped_elements.shape}")
# #         unflattened_weights.append(tf.convert_to_tensor(reshaped_elements, dtype=tf.float64))
# #         current_index += num_elements
# #     return unflattened_weights
...
# # def get_model_parameters(model: CNN) -> LogRegParams:
# #     """Returns the parameters of a CNN model. """
# #     params = model.get_weights()
# #     return [param.numpy() for param in params]

# # def set_model_params(model: CNN, params: LogRegParams) -> CNN:
# #     """Sets the parameters of a CNN model. """
# #     tensor_params = [tf.convert_to_tensor(param, dtype=tf.float32) for param in params]
# #     model.set_weights(tensor_params)
# #     return model


# # def get_flat_weights(model: CNN) -> Tuple[np.ndarray, List[Tuple[int, ...]]]:
# #     """ Get model weights, convert and flatten nested tensor to a numpy array, original shape is saved. """
# #     params = model.get_weights()
# #     original_shapes = [tensor.shape for tensor in params]
# #     flattened_weights = np.concatenate([weight_tensor.numpy().flatten() for weight_tensor in params])
# #     return flattened_weights, original_shapes


# # def unflatten_weights(flattened_weights: np.ndarray, original_shapes: List[Tuple[int, ...]]) -> List[np.ndarray]:
# #     """Convert flat numpy array into the model's original nested numpy arrays. """
# #     unflattened_weights = []
# #     current_index = 0
# #     for shape in original_shapes:
# #         num_elements = np.prod(shape)
# #         current_elements = flattened_weights[current_index : current_index + num_elements]
# #         reshaped_elements = np.reshape(current_elements, shape)
# #         unflattened_weights.append(reshaped_elements)
# #         current_index += num_elements
# #     return unflattened_weights


# # def pad_to_power_of_2(flat_params: np.ndarray, target_length=2**20, weight_decimals=8) -> Tuple[np.ndarray, int]:
# #     """
# #     - Client Side
# #     - Before encryption of local weights:
# #     - Pad flat weights to nearest 2^n, original length is saved. 
...
# #     if pad_length < 0:
# #         raise ValueError("The given target_length is smaller than the current parameter list length.")
# #     random_padding = np.random.randint(-10**weight_decimals, 10**weight_decimals + 1, pad_length)
# #     padded_params = np.concatenate([flat_params, random_padding])
# #     return padded_params, len(flat_params)


# # def remove_padding(padded_params: np.ndarray, original_length: int) -> np.ndarray:
# #     """
# #     - Client Side
# #     - After receiving decrypted model updates:
# #     - Remove the padding that was added during encryption 
...
#     return padded_params[:original_length]
def remove_padding(padded_params, original_length):
    # Convert padded_params to native Python floats
    if padded_params is not None:
        padded_params = [float(param) for param in padded_params]
    return padded_params[:original_length]
