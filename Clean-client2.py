...
#                        help='Dataset to use')
#     parser.add_argument('--num_clients', type=int, default=6,  # ADD THIS LINE
...
#             self.metrics_dir = f"metrics/{self.session_id}"
...
#             # NOW initialize the bucket detector
#             # self.bucket_detector = AdvancedBucketDetector(
#             #     n_features=self.model_length,  # Full model length for reference
#             #     sample_size=1000,              # Actual size of samples we'll analyze
#             #     memory_size=50,
#             #     sensitivity=2.0,
#             #     vote_threshold=0.5,
#             #     pca_components=5,
...
#             # Find and replace the AdvancedBucketDetector initialization
#             self.bucket_detector = AdvancedBucketDetector(
...
#             # if is_poisoning:
#             #     print(f"Client {self.client_id} - Applying AGGRESSIVE poisoning attack in round {self.current_round}")
...
#             #         'timestamp': time.time()
#             #     }
#             #     self.attack_metrics.append(attack_event)
...
#             #     # Override with stronger poisoning - THIS IS THE KEY CHANGE
#             #     poisoning_config['poison_ratio'] = 1.0  # Force 100% poisoning
#             #     if poisoning_config['attack_type'] == 'label_flip':
#             #         # For label_flip attacks, let's target class 1 consistently
#             #         poisoning_config['target_class'] = 1
                
#             #     # Debug print
#             #     print(f"Client {self.client_id} - Modified attack parameters: {poisoning_config}")
...
#             #         **poisoning_config
#             #     )
#             # In the fit() method, replace the poisoning section with:
#             if is_poisoning:
#                 print(f"Client {self.client_id} - Applying poisoning attack in round {self.current_round}")
#                 attack_config = self.experiment.get_attack_config()
                
#                 # Import strong attacks
#                 from strong_attacks import FangAttack, MinMaxAttack
                
#                 # Apply different attack types
#                 attack_type = attack_config.get('type', 'label_flip')
                
#                 if attack_type == 'fang':
#                     fang_attack = FangAttack(attack_strength=2.0)
#                     X_train_poisoned, y_train_poisoned = fang_attack.apply_attack(
#                         self.original_X_train, self.original_y_train
#                     )
#                 elif attack_type == 'min_max':
#                     minmax_attack = MinMaxAttack(target_class=1)
#                     X_train_poisoned, y_train_poisoned = minmax_attack.apply_attack(
#                         self.original_X_train, self.original_y_train
#                     )
#                 else:
#                     # Original poisoning method
#                     poisoning_config = attack_config.copy()
#                     if 'type' in poisoning_config:
#                         poisoning_config['attack_type'] = poisoning_config.pop('type')
...
#                 # Print data stats for verification
#                 if len(y_train_poisoned) > 0:
#                     print(f"Client {self.client_id} - Poisoned {len(y_train_poisoned)} samples")
#                     unique, counts = np.unique(y_train_poisoned, return_counts=True)
#                     print(f"Client {self.client_id} - Label distribution after poisoning: {dict(zip(unique, counts))}")
...
#             # FIXED: Handle training_history properly
#             # Check if it's a dictionary or History object
...
#                     # If it's a list, take the first element; if it's a single value, return it
...
#             self.save_metrics_to_file(training_metrics, "training_metrics")
...
#                         'metrics': metrics,
#                         'thresholds': {
#                             k: self.bucket_detector._dynamic_threshold(k)
...
#                     self.save_metrics_to_file(bucket_metrics, "bucket_evaluation_metrics")
...
#             self.save_metrics_to_file(eval_metrics, "evaluation_metrics")
...
#                 'round': self.current_round,
#                 'client_id': self.client_id,
#                 'sigma_global_sq': sigma_global_sq,
#                 'rho_sq': rho_sq,
#                 'alpha': alpha,
#                 'timestamp': time.time()
#             }
...
#             # Save metrics
#             self.save_metrics_to_file(variance_metrics, "variance_analysis")
...
#             self.save_metrics_to_file(global_model_metrics, "global_model_metrics")
            
#             # Save detailed per-class metrics if possible
#             if not has_nan:
#                 per_class_metrics = {
#                     'round': self.current_round,
#                     'client_id': self.client_id,
#                     'timestamp': time.time(),
#                     'classification_report': classification_report(self.y_test, predicted, output_dict=True)
#                 }
#                 self.save_metrics_to_file(per_class_metrics, "per_class_metrics")
...
#                 json_file = f"{self.metrics_dir}/{filename_prefix}_{self.client_id}.jsonl"
...
#                 csv_file = f"{self.metrics_dir}/{filename_prefix}_all.csv"
...
#                 print(f"Metrics saved to {json_file} and {csv_file}")
...
#             """Save a summary of all metrics at the end of the session"""
#             try:
#                 summary = {
...
#                     'attacks_performed': len(self.attack_metrics),
#                     'bucket_evaluations': len(self.bucket_evaluation_metrics),
#                     'is_malicious': self.client_id in self.experiment.malicious_client_ids,
#                     'attack_config': self.experiment.get_attack_config() if self.client_id in self.experiment.malicious_client_ids else None
#                 }
                
#                 summary_file = f"{self.metrics_dir}/session_summary_{self.client_id}.json"
...
#     # Start Flower client with our CnnClient implementation
...
#             # Also save memory usage to metrics
#             memory_metrics = {
#                 'client_id': client.client_id,
#                 'session_id': client.session_id,
#                 'memory_usage_mb': memory_usage_total,
#                 'timestamp': time.time()
#             }
#             client.save_metrics_to_file(memory_metrics, "memory_usage")
